...
def massal():
    ...
    for t in range(tanya_total):
        t +=1
        print('%s║'%(B))
        id=input(f"%s╠══[%s•%s] %sMasukkan User ID {P}{t}  : {B}"%(K,P,K,P))
        try:
            url= requests.Session().get("https://graph.facebook.com/%s?fields=friends.limit(50000)&access_token=%s"%(id,token),cookies=cookie)
...
def publik():
    ...
    id=input(f"{K}╠══[•] {P}Masukan user id : {B}")
    try:
        url= requests.Session().get("https://graph.facebook.com/%s?fields=friends.limit(50000)&access_token=%s"%(id,token),cookies=cookie)
        z=json.loads(url.text)
        for i in z['friends']['data']:
...
def followerss():
    ...
    id=input(f"{K}╠══[•] {P}Masukan user id : {B}")
    try:
        for i in requests.Session().get("https://graph.facebook.com/%s/subscribers?limit=50000&access_token=%s"%(id,token),cookies=cookie).json()["data"]:
            uid = i["id"]
            nama = i["name"]
            id.append(uid+"<=>"+nama)
    except KeyError:
